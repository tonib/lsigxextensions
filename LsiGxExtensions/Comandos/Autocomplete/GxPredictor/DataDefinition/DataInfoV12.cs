...
namespace LSI.Packages.Extensiones.Comandos.Autocomplete.GxPredictor.DataDefinition
{
	...
	public class DataInfoV12 : DataInfo
    {
        /// <summary>
		...
		public DataInfoV12(int maxTexthash) : base(maxTexthash)
        {
		...
		{
			if (dataType.ExtendedTypeName != null)
			{
				// Return a hash for the extended (BC/SDT/External object/etc) type name
				uint hash = MurmurHash2.Hash(dataType.ExtendedTypeName);
		...
		override protected void SetupColumns()
		{
			...
			{
				SetSdtsDataType = false,
				LineBreaksAsKeyword = false
			};
			...
			AddColumn(
				new ColumnStringCategory("DataType")
				{
			...
			AddColumn(
				new ColumnIntCategory("Length", LengthBuckets, true)
				{
					FeatureExtract = (token, context) => {
						if (token.DataType.Type == eDBType.NONE)
							return null;
			...
			AddColumn(
				new ColumnIntCategory("Decimals", DecimalBuckets, true)
				{
					FeatureExtract = (token, context) => {
						if (token.DataType.Type == eDBType.NONE)
							return null;
			...
			for (int i = 0; i < NHashColumns; i++)
			{
				...
				AddColumn(
					new ColumnStringHashMurmur("NameHash" + index, MaxTextHash, true)
					{
			...
			AddColumn(
				new ColumnStringCategory("CtxParmType")
				{
			...
			AddColumn(
				new ColumnIntCategory("CtxParmLength", LengthBuckets, true)
				{
					FeatureExtract = (token, context) =>
					{
			...
			AddColumn(
				new ColumnIntCategory("CtxParmDecimals", DecimalBuckets, true)
				{
					FeatureExtract = (token, context) =>
					{
						if (context.ParmType.Type == eDBType.NONE)
			...
			for (int i = 0; i < NHashColumns; i++)
			{
				...
				int index = i;
				AddColumn(
					new ColumnStringHashMurmur("CtxParmNameHash" + index, MaxTextHash, true)
					{
						FeatureExtract = (token, context) =>
						{
							if (context.ParmType.Type == eDBType.NONE)
        ...
        /// Model version
        /// </summary>
        override public int ModelVersion { get { return 12; } }
